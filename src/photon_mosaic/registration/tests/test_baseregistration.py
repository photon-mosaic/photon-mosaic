...
from photon_mosaic.registration.baseregistration import (
    BaseRegistration,
    BaseRegistrationEpoch,
)
...
class TestBaseRegistration:
    ...
    def test_init_custom_sampling_frequency(self, imaging):
        reg = BaseRegistration(imaging, sampling_frequency=15.0)
    ...
    def test_init_custom_dtype(self, imaging):
        ...
        BaseRegistration(imaging, dtype=np.float32)

    def test_init_rejects_non_imaging(self):
        with pytest.raises(AssertionError, match="must be a BaseImaging"):
            BaseRegistration("not_an_imaging_object")

    def test_init_rejects_dict(self):
        with pytest.raises(AssertionError):
            BaseRegistration({"data": [1, 2, 3]})


# ---------------------------------------------------------------------------
# BaseRegistration – multi-epoch
# ---------------------------------------------------------------------------


class TestBaseRegistrationMultiEpoch:
    @pytest.fixture()
    def imaging(self):
...
class TestBaseRegistrationEpoch:
    ...
    def epoch(self, imaging):
        ...
        return BaseRegistrationEpoch(parent_epoch)

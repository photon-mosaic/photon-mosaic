...
from photon_mosaic.core.arrayimaging import ArrayImaging, NumpyImagingEpoch, NumpyRois
...
def test_numpyimaging_multi_plane_4d_video():
    # 4D video: (frames, height, width, planes)
    video = np.zeros((4, 6, 7, 3), dtype=np.float32)

    im = ArrayImaging(imaging_series=video, sampling_frequency=20.0)
    assert im.get_num_planes() == 3
